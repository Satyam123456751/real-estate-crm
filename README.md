...
JWT_SECRET=any_random_secret_string   ← change this!
PORT=5000
```
...
psql -U postgres -d realestate_crm -f config/schema.sql
```
...
| Error | Fix |
|-------|-----|
| `ECONNREFUSED` on DB | PostgreSQL not running. Start pgAdmin or run `pg_ctl start` |
